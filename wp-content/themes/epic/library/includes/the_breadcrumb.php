...
function the_breadcrumb(){
	...
	global $post;
	echo '<p id="breadcrumb-container"><span class="breadcrumb_info">'.__('You are here:', 'Epic').'</span> <a href="'.home_url().'">';
		bloginfo('name');
		echo "</a>";
...
function simple_breadcrumb_case($der_post){
	...
	if(is_single()){
		...
		if (is_attachment()){
			...
			{
				$terms = get_the_term_list( get_the_ID(), 'portfolio_category', '', '&raquo;', '' );				
				
				$terms = explode('&raquo;',$terms);
			...
			{
				$terms = get_the_term_list( get_the_ID(), 'events', '', '&raquo;', '' );				
